...
<main>
  ...
  <section class="content">
    ...
    {:else}
      ...
      <form on:submit|preventDefault={refresh} class="search"><input bind:value={query} placeholder="owner/repository" aria-label="Search repositories" />{#if query}<button type="button" class="secondary" on:click={clearSearch}>Cancel</button>{/if}<button><Search size={14} />Search</button></form>
      {#if error}<p class="error">{error}</p>{/if}
      {#if dashboard}
        <div class="kpis">
